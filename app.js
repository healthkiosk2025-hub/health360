    ...
    const JWT_SECRET = process.env.JWT_SECRET || "your-default-secret"; // Fallback for local development
    ...
    // OTP storage (in-memory, as before) - Note: This will reset with each function invocation on Vercel
    const otpStorage = {};
    const dataStorage = {};
          ...
          if (type === "mother") {
            ...
            await connection.query(
              `INSERT INTO mothers (
                ...
                ifsc_code, aadhaar_no, mobile, admin_id, created_date, created_at, updated_at, lmp_date, delivery_expected
              ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
              [
                data.mother_id,
                data.name,
                ...
                data.ifsc_code,
                data.aadhaar_no,
                data.phone, // Assuming 'mobile' is same as 'phone'
                data.admin_id,
                data.created_date,
                new Date().toISOString(),
                new Date().toISOString(),
                data.lmp_date,
                data.delivery_expected,
              ]
            );
            ...
            await connection.query(
              `INSERT INTO children (
                child_id, name, aadhaar_no, dob, age, mother_id, email, admin_id, password, created_at, updated_at
              ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
              [
                data.child_id,
                data.name,
            ...
            return res.json({ message: "Child record saved" });
          }
      ...
      try {
        ...
        try {
          ...
          if (collection === "admins") {
            idField = "admin_id";
            tableName = "admins";
          } else if (collection === "panchayats") {
            idField = "panchayat_id";
            tableName = "panchayats";
          } else if (collection === "mothers") {
            idField = "mother_id";
            tableName = "mothers";
          ...
          const [rows] = await connection.query(
            `SELECT ${idField} FROM ${tableName} WHERE ${idField} LIKE ?`,
            [`${prefix}%`]
          );
    ...
    app.post("/update-child", async (req, res) => {
      const { child_id, name, aadhaar_no, dob, age, mother_id, admin_id, email, password } = req.body;
      ...
      try {
        ...
        try {
          ...
          const params = [name, aadhaar_no, age, mother_id, admin_id, email, updateData.updated_at];
    ...
    app.post("/update-mother", async (req, res) => {
      const {
        ...
        account_no, ifsc_code, bank_name, admin_id, lmp_date, delivery_expected, password
      } = req.body;
      ...
      try {
        ...
        try {
          ...
          if (isEditing) {
            ...
            await connection.query(
              "INSERT INTO panchayats (panchayat_id, panchayat_name, location, admin_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)",
              [panchayat_id, panchayat_name, location, admin_id, now, now]
            );
          }
      ...
      try {
        ...
        try {
          ...
          if (vaccineRows.length > 0) {
            ...
            await connection.query(
              "INSERT INTO children_vaccination (child_id, vaccine, status, date_schedule, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)",
              [childId, vaccine, status ? "True" : "False", date_schedule || null, now, now]
            );
          }
